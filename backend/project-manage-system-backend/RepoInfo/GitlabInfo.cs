...
namespace project_manage_system_backend.RepoInfo
{
    public class GitlabInfo : RepoInfoBase
    {
        ...
        public override async Task<List<ResponseFileDto>> RequestFileSet(Repo repo)
        {
            try
            {
                string apiUrl = $"https://sgit.csie.ntut.edu.tw/gitlab/api/v4/projects/{repo.RepoId}/repository/tree?{token}";
                ...
                {
                    var content = await response.Content.ReadAsStringAsync();
                    var files = JsonSerializer.Deserialize<List<ResponseFileDto>>(content);
                ...
                {
                    // 處理錯誤情況，可能是權限不足等
                    throw new Exception($"Failed to get file set. Status code: {response.StatusCode}");
                }
            }
            catch (Exception ex)
            {
                // 在實際應用中，你可能需要更詳細的錯誤處理邏輯
                throw new Exception($"Error in RequestFileSet: {ex.Message}");
            }
        }
    }
}
