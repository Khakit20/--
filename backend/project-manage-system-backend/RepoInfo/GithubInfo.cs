...
namespace project_manage_system_backend.RepoInfo
{
    public class GithubInfo : RepoInfoBase
    {
        ...
        public override async Task<List<ResponseFileDto>> RequestFileSet(Repo repo)
        {
            return await GetRepoContents(repo);
        }

        private async Task<List<ResponseFileDto>> GetRepoContents(Repo repo, string path = "")
        {
            try
            {
                string url = $"https://api.github.com/repos/{repo.Owner}/{repo.Name}/contents/{path}";
                var response = await _httpClient.GetAsync(url);

                if (response.IsSuccessStatusCode)
                {
                    var content = await response.Content.ReadAsStringAsync();
                    var contents = JsonSerializer.Deserialize<List<ResponseFileDto>>(content);
                ...
                {
                    // 處理錯誤情況，可能是權限不足等
                    throw new Exception($"Failed to get contents. Status code: {response.StatusCode}");
                }
            }
            catch (Exception ex)
            {
                // 在實際應用中，你可能需要更詳細的錯誤處理邏輯
                throw new Exception($"Error in GetRepoContents: {ex.Message}");
            }
        }
    }
}
